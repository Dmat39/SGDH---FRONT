...
import {
  ...
  Visibility, VisibilityOff, Accessible, MedicalServices,
} from "@mui/icons-material";
import type { CentroOMAPED } from "./MapaOMAPED";

const MapaOMAPED = dynamic(() => import("./MapaOMAPED"), {
  ssr: false,
  loading: () => (
...
const FILTROS_CONFIG = [
  ...
  { id: "servicios", label: "Servicios", opciones: [{ id: "menos5", label: "Menos de 5" }, { id: "5-10", label: "5 - 10" }, { id: "mas10", label: "Más de 10" }] },
];

const CENTROS_EJEMPLO: CentroOMAPED[] = [
  { id: "1", codigo: "OMAPED-001", nombre: "OMAPED Central SJL", direccion: "Av. Próceres 1234, Zarate", jurisdiccion: "ZARATE", coordenadas: { latitud: -11.9650, longitud: -76.9950 }, beneficiarios: 180, servicios: 12, responsable: { nombre: "Luis García Mendoza", telefono: "987654321" }, estado: "activo" },
  { id: "2", codigo: "OMAPED-002", nombre: "Centro de Atención Campoy", direccion: "Jr. Los Jazmines 456, Campoy", jurisdiccion: "CAMPOY", coordenadas: { latitud: -11.9720, longitud: -76.9900 }, beneficiarios: 95, servicios: 8, responsable: { nombre: "Rosa Mendoza Quispe", telefono: "912345678" }, estado: "activo" },
  { id: "3", codigo: "OMAPED-003", nombre: "OMAPED Mangomarca", direccion: "Calle Los Rosales 789, Mangomarca", jurisdiccion: "MANGOMARCA", coordenadas: { latitud: -11.9680, longitud: -77.0020 }, beneficiarios: 120, servicios: 10, responsable: { nombre: "Carmen Huamán Torres", telefono: "945678123" }, estado: "activo" },
  { id: "4", codigo: "OMAPED-004", nombre: "Centro Inclusivo Canto Rey", direccion: "Av. Central 321, Canto Rey", jurisdiccion: "CANTO REY", coordenadas: { latitud: -11.9750, longitud: -76.9850 }, beneficiarios: 85, servicios: 6, responsable: { nombre: "Pedro Pérez Sánchez", telefono: "956789234" }, estado: "activo" },
  { id: "5", codigo: "OMAPED-005", nombre: "OMAPED Huayrona", direccion: "Jr. Las Palmeras 654, Huayrona", jurisdiccion: "HUAYRONA", coordenadas: { latitud: -11.9630, longitud: -77.0080 }, beneficiarios: 55, servicios: 4, responsable: { nombre: "Ana Castillo Ramos", telefono: "967890345" }, estado: "inactivo" },
];
...
export default function OMAPEDMapaPage() {
  ...
  const [capasVisibles, setCapasVisibles] = useState({ sectores: true, jurisdicciones: false, centros: true });
  const [centroSeleccionado, setCentroSeleccionado] = useState<CentroOMAPED | null>(null);
    ...
    <Box sx={{ display: "flex", flexDirection: "column", height: "calc(100vh - 64px + 32px)", marginLeft: "-16px", marginRight: "-16px", marginBottom: "-16px", marginTop: "-16px", overflow: "hidden", "@media (min-width: 768px)": { height: "calc(100vh - 64px + 48px)", marginLeft: "-24px", marginRight: "-24px", marginBottom: "-24px", marginTop: "-24px" } }}>
      ...
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", px: 2, py: 1, bgcolor: "white", borderBottom: "1px solid", borderColor: "divider", zIndex: 10 }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          ...
          <Typography variant="h6" fontWeight="bold">Mapa OMAPED</Typography>
        </Box>
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
          <Box sx={{ display: "flex", alignItems: "center", bgcolor: "grey.100", px: 2, py: 0.5, borderRadius: 1 }}>
            <Typography variant="body2" color="text.secondary">
              ...
              {hayFiltrosActivos() && totalFiltrados < centros.length && <Typography component="span" variant="caption" sx={{ ml: 1, color: "#2196f3" }}>(filtrado de {centros.length})</Typography>}
            </Typography>
          </Box>
          <Tooltip title="Ajustar cantidad visible">
            <Box onClick={(e) => setLimiteAnchorEl(e.currentTarget)} sx={{ display: "flex", alignItems: "center", gap: 0.5, bgcolor: "#2196f3", color: "white", px: 1.5, py: 0.5, borderRadius: 1, cursor: "pointer", "&:hover": { bgcolor: "#1976d2" } }}>
          ...
          <Popover open={limitePopoverOpen} anchorEl={limiteAnchorEl} onClose={() => setLimiteAnchorEl(null)} anchorOrigin={{ vertical: "bottom", horizontal: "center" }} transformOrigin={{ vertical: "top", horizontal: "center" }}>
            <Box sx={{ p: 2, width: 300 }}>
              ...
              <Box sx={{ px: 1 }}><Slider value={Math.min(limiteVisible, totalFiltrados)} onChange={(_, value) => setLimiteVisible(value as number)} min={10} max={totalFiltrados || 500} step={10} valueLabelDisplay="auto" sx={{ color: "#2196f3" }} /></Box>
              <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap", mt: 2 }}>
                {LIMITE_OPCIONES.filter(op => op <= (totalFiltrados || 500)).map((opcion) => (<Chip key={opcion} label={opcion} size="small" onClick={() => setLimiteVisible(opcion)} sx={{ bgcolor: limiteVisible === opcion ? "#2196f3" : "grey.200", color: limiteVisible === opcion ? "white" : "text.primary" }} />))}
                <Chip label={`Todos (${totalFiltrados})`} size="small" onClick={() => setLimiteVisible(totalFiltrados || 500)} sx={{ bgcolor: limiteVisible >= totalFiltrados ? "#2196f3" : "grey.200", color: limiteVisible >= totalFiltrados ? "white" : "text.primary" }} />
              </Box>
            </Box>
          </Popover>
        </Box>
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
          ...
          <Tooltip title="Filtros">
            <IconButton size="small" onClick={() => setFilterOpen(true)} sx={{ bgcolor: filtrosActivos > 0 ? "#2196f3" : "transparent", color: filtrosActivos > 0 ? "white" : "inherit", "&:hover": { bgcolor: filtrosActivos > 0 ? "#1976d2" : "action.hover" } }}>
          ...
          <Popover open={capasPopoverOpen} anchorEl={capasAnchorEl} onClose={() => setCapasAnchorEl(null)} anchorOrigin={{ vertical: "bottom", horizontal: "right" }} transformOrigin={{ vertical: "top", horizontal: "right" }} PaperProps={{ sx: { borderRadius: 2, boxShadow: "0 4px 20px rgba(0,0,0,0.15)" } }}>
            <Box sx={{ width: 280 }}>
              ...
              <Box sx={{ p: 1 }}>
                {[{ key: "sectores", label: "Comunas", desc: "Sectores del distrito", color: "#2196f3" }, { key: "jurisdicciones", label: "Jurisdicciones", desc: "Límites territoriales", color: "#34b429" }, { key: "centros", label: "Centros OMAPED", desc: "Ubicación de centros", color: "#1E293B" }].map((capa) => (
                  <Box key={capa.key} sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", p: 1.5, borderRadius: 1 }}>
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                      <Box sx={{ width: 32, height: 32, borderRadius: capa.key === "centros" ? "50%" : 1, bgcolor: capa.color, display: "flex", alignItems: "center", justifyContent: "center", opacity: capasVisibles[capa.key as keyof typeof capasVisibles] ? 1 : 0.4 }}>
                        {capa.key === "centros" ? <Accessible sx={{ fontSize: 18, color: "white" }} /> : <Box sx={{ width: 16, height: 16, bgcolor: "rgba(255,255,255,0.9)", borderRadius: 0.5 }} />}
                      </Box>
              ...
              <Box sx={{ p: 1.5, display: "flex", gap: 1 }}>
                <Button size="small" variant="outlined" fullWidth onClick={() => setCapasVisibles({ sectores: true, jurisdicciones: true, centros: true })} startIcon={<Visibility sx={{ fontSize: 16 }} />} sx={{ textTransform: "none", borderColor: "grey.300", color: "text.secondary" }}>Mostrar todas</Button>
                <Button size="small" variant="outlined" fullWidth onClick={() => setCapasVisibles({ sectores: false, jurisdicciones: false, centros: false })} startIcon={<VisibilityOff sx={{ fontSize: 16 }} />} sx={{ textTransform: "none", borderColor: "grey.300", color: "text.secondary" }}>Ocultar todas</Button>
              </Box>
            </Box>
          </Popover>
        </Box>
      ...
      <Box sx={{ flex: 1, position: "relative", display: "flex" }}>
        {centroSeleccionado && (
          <Paper elevation={4} sx={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 350, zIndex: 1000, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <Box sx={{ bgcolor: "#2196f3", color: "white", p: 2, display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
              <Box sx={{ flex: 1, pr: 1 }}>
                <Typography variant="subtitle1" fontWeight="bold" sx={{ lineHeight: 1.2 }}>{centroSeleccionado.nombre}</Typography>
                <Typography variant="caption" sx={{ opacity: 0.9 }}>Código: {centroSeleccionado.codigo}</Typography>
              </Box>
              <Box sx={{ display: "flex", gap: 0.5 }}>
                ...
                <IconButton size="small" sx={{ color: "white" }} onClick={() => setCentroSeleccionado(null)}><Close fontSize="small" /></IconButton>
              </Box>
            </Box>
            <Box sx={{ flex: 1, overflow: "auto", p: 2 }}>
              <Box sx={{ mb: 2 }}><Box sx={{ display: "inline-block", px: 1.5, py: 0.5, borderRadius: "12px", backgroundColor: centroSeleccionado.estado === "activo" ? "#dcfce7" : "#fee2e2", color: centroSeleccionado.estado === "activo" ? "#166534" : "#991b1b", fontSize: "0.75rem", fontWeight: 600 }}>{centroSeleccionado.estado === "activo" ? "ACTIVO" : "INACTIVO"}</Box></Box>
              <Box sx={{ mb: 3 }}><Typography variant="overline" color="text.secondary" fontWeight="bold">Dirección</Typography><Box sx={{ display: "flex", alignItems: "flex-start", gap: 1, mt: 0.5 }}><Place fontSize="small" color="action" /><Typography variant="body2">{centroSeleccionado.direccion}</Typography></Box><Typography variant="caption" color="text.secondary" sx={{ ml: 3.5 }}>Jurisdicción: {centroSeleccionado.jurisdiccion}</Typography></Box>
              <Divider sx={{ mb: 2 }} />
              <Box sx={{ mb: 3 }}><Typography variant="overline" color="text.secondary" fontWeight="bold">Responsable</Typography><Box sx={{ display: "flex", alignItems: "center", gap: 2, mt: 1 }}><Avatar sx={{ bgcolor: "#1E293B", width: 40, height: 40 }}><Person /></Avatar><Box><Typography variant="body2" fontWeight="medium">{centroSeleccionado.responsable.nombre}</Typography><Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}><Phone fontSize="inherit" sx={{ color: "text.secondary", fontSize: 14 }} /><Typography variant="caption" color="text.secondary">{centroSeleccionado.responsable.telefono}</Typography></Box></Box></Box></Box>
              <Divider sx={{ mb: 2 }} />
              <Box sx={{ mb: 3 }}><Typography variant="overline" color="text.secondary" fontWeight="bold">Estadísticas</Typography><Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2, mt: 1 }}><Box sx={{ bgcolor: "#e3f2fd", p: 1.5, borderRadius: 1, textAlign: "center" }}><Group sx={{ color: "#2196f3", mb: 0.5 }} /><Typography variant="h5" fontWeight="bold" color="#2196f3">{centroSeleccionado.beneficiarios}</Typography><Typography variant="caption" color="text.secondary">Beneficiarios</Typography></Box><Box sx={{ bgcolor: "#fce4ec", p: 1.5, borderRadius: 1, textAlign: "center" }}><MedicalServices sx={{ color: "#e91e63", mb: 0.5 }} /><Typography variant="h5" fontWeight="bold" color="#e91e63">{centroSeleccionado.servicios}</Typography><Typography variant="caption" color="text.secondary">Servicios</Typography></Box></Box></Box>
              <Box sx={{ bgcolor: "grey.50", p: 1.5, borderRadius: 1 }}><Typography variant="caption" color="text.secondary"><strong>Coordenadas:</strong> {centroSeleccionado.coordenadas.latitud.toFixed(6)}, {centroSeleccionado.coordenadas.longitud.toFixed(6)}</Typography></Box>
            </Box>
          </Paper>
        )}
        <Box sx={{ flex: 1, position: "relative" }}>
          {loading ? (<Box sx={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%" }}><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600"></div></Box>) : (
            <MapaOMAPED centros={centrosFiltrados} centroSeleccionado={centroSeleccionado} onCentroClick={setCentroSeleccionado} limiteVisible={limiteVisible} filterOpen={filterOpen} capasVisibles={capasVisibles} />
          )}
        </Box>
      </Box>
      ...
      <Drawer anchor="right" open={filterOpen} onClose={() => setFilterOpen(false)} PaperProps={{ sx: { width: 320, maxWidth: "100%" } }}>
        ...
        <Box sx={{ display: "flex", gap: 1, p: 2, borderBottom: "1px solid", borderColor: "divider" }}><Button variant="outlined" fullWidth onClick={limpiarFiltros} disabled={filtrosActivos === 0}>LIMPIAR</Button><Button variant="contained" fullWidth onClick={() => setFilterOpen(false)} sx={{ bgcolor: "#2196f3", "&:hover": { bgcolor: "#1976d2" } }}>APLICAR</Button></Box>
        <Box sx={{ flex: 1, overflow: "auto" }}>
          {FILTROS_CONFIG.map((categoria) => (
            <Accordion key={categoria.id} expanded={expandedAccordion === categoria.id} onChange={(_, isExpanded) => setExpandedAccordion(isExpanded ? categoria.id : false)} disableGutters elevation={0} sx={{ "&:before": { display: "none" }, borderBottom: "1px solid", borderColor: "divider" }}>
              <AccordionSummary expandIcon={<ExpandMore />}><Box sx={{ display: "flex", alignItems: "center", gap: 1 }}><Typography fontWeight="medium">{categoria.label}</Typography>{(filtrosSeleccionados[categoria.id]?.length || 0) > 0 && <Box sx={{ bgcolor: "#2196f3", color: "white", borderRadius: "50%", width: 20, height: 20, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12 }}>{filtrosSeleccionados[categoria.id]?.length}</Box>}</Box></AccordionSummary>
              <AccordionDetails sx={{ pt: 0 }}><FormGroup>{categoria.opciones.map((opcion) => (<FormControlLabel key={opcion.id} control={<Checkbox size="small" checked={filtrosSeleccionados[categoria.id]?.includes(opcion.id) || false} onChange={(e) => handleFilterChange(categoria.id, opcion.id, e.target.checked)} sx={{ color: "#2196f3", "&.Mui-checked": { color: "#2196f3" } }} />} label={<Typography variant="body2">{opcion.label}</Typography>} />))}</FormGroup></AccordionDetails>
            </Accordion>
          ))}
